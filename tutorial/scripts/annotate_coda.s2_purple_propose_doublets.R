...
# 2. [later] merge similar subclones
# 
...
if (sample_name == 'DCIS67T') {merge_to_k = 8}
if (sample_name == 'BCMDCIS69') {merge_to_k = 8}
if (sample_name == 'IBC38HJ') {merge_to_k = 4}
message(merge_to_k)
...
draw( plotHeatmap(objd, label = merge_for_z, group = 'run', 
...
write_csv(x = data.frame(old = levels(objd[[merge_for_z]]), 
                         new= levels(objd[[merge_for_z]])), file=f_newclones)
stop(
  'Manually propose C*, DC*, and Z groups.'
)
...
pdf(file.path(dir_res, sprintf('plotHeatmap.%s_to_merge.pdf', merge_for_z)), 
    width = 12, height = 5, onefile = T, useDingbats = F)

objd <- calcConsensus(objd, consensus_by = merge_for_z, assay = 'integer')
objd <- runConsensusPhylo(objd)
draw( plotHeatmap(objd, label = merge_for_z, group = 'clones', 
                  consensus=T, label_colors = pal_cna_merge_z, 
                  assay = 'integer') )
