...
#------ prepare colors ------
pal_list <- list(
  'clones' = new_palette_D(levels(objd[['clones']])), 
  'subclones' = new_palette_D(levels(objd[['subclones']]), 'ironMan'), 
  'seurat_clusters' = new_palette_D(levels(objd[['seurat_clusters']]), 'circus')
)

#------------------- ~~~ Merge similar clones ~~~ -------------------  
n_bin_diff_tolerate <- 3 # requires clones showing >3 bins diff
#------ Specify parameters ------
# para for merging similar clones
merge_to_k = NULL

if (sample_name == 'DCIS28T') {merge_to_k = 9}
if (sample_name == 'DCIS35T') {merge_to_k = 4}
if (sample_name == 'DCIS36T') {merge_to_k = 13}
if (sample_name == 'DCIS41T') {merge_to_k = 17}
if (sample_name == 'DCIS51T') {merge_to_k = 5}
if (sample_name == 'DCIS68T') {merge_to_k = NULL} # no need
...
pdf(file.path(dir_res, sprintf('plotHeatmap.%s_before_merge.pdf', merge_for_z)), 
    width = 12, height = 5, onefile = F, useDingbats = F)
draw( plotHeatmap(objd, group = 'run', 
                  consensus=T, 
                  label = merge_for_z, 
                  label_colors = pal_list[merge_for_z],
                  assay = 'integer') )
dev.off()
...
if (T) {
  ...
  if (min(ck_css) < 0) {
    ck_css_gains <- 1 * (ck_css >= 3/2)
    ck_css_loses <- -1 * (ck_css <= 1/2)
  } else {
